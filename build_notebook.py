...
md(
    ...
    "qbt/kalshi.py       KalshiPanel -- Kalshi event-contract odds, forward-looking",
    ...
    "```",
    "",
    "The five newer panels all share the same `as_of()` look-ahead firewall as",
    "`PricePanel` -- a strategy that reads them is structurally unable to see a",
    "filing, a macro release, an options snapshot, or a Kalshi quote before it",
    "actually happened. Section 4 covers them and the strategies built on them.",
    "",
...
code(
    ...
    "    InverseVolWeighted, KalshiEventRegimeFilter, KalshiPanel,",
    "    KalshiRepository, LiveSignalRunner, MacroRegimeFilter, MacrosPanel,",
    ...
    "    ParameterSweep, PortfolioState, PricePanel, RiskGate, RiskParityAllocation,",
...
md(
    ...
    "",
    "**Incomplete last bars are dropped at fetch time.** Early in the",
    "morning, yfinance can return the previous session's row with `open` and",
    "`volume` but no `close` (seen live, 2026-09-23 08:19 ET, on all 30",
    "symbols). As the last bar, that row made `last_close()` all NaN -- so the",
    "live cycle valued every held position at $0 and tripped the drawdown",
    "breaker on an account that hadn't lost anything. `OpenBBRepository.fetch`",
    "now drops any trailing bar where a symbol that closed the bar before has",
    "no close, reports it in `repo.last_trimmed` (the next cell prints it),",
    "never caches the incomplete response, and raises if more than",
    "`max_trim_bars` (3) bars in a row are incomplete. Gaps *inside* the",
    "history (halts, listing dates) are real data and are left alone.",
)
...
code(
    ...
    "    # fetch() drops trailing bars the provider hadn't finished (see the",
    "    # note above) -- read it now, before the benchmark fetch overwrites it.",
    "    for _d, _syms in repo.last_trimmed:",
    "        print(f'dropped incomplete bar {_d.date()}: no close yet for '",
    "              f'{len(_syms)} symbol(s) -- panel ends on the last complete bar')",
...
md(
    "---",
    "## 4. Beyond price: fundamentals, macro, corporate filings, options, Kalshi",
    "",
    "Everything above is a pure function of price. Five more data types plug",
    "into the exact same `Strategy` protocol -- `target_weights` just grows",
    "four more optional parameters (`fundamentals`, `macros`, `corps`,",
    "`options`, `kalshi`), all `None` by default, so nothing above this cell",
    "changes:",
    "",
    ...
    "  reading, keyed by FRED's *release* date -- reacts after the fact.",
    ...
    "- **`KalshiPanel`** (`qbt/kalshi.py`) -- market-implied odds on the *same*",
    "  scheduled releases `MacrosPanel` reads off FRED (CPI, Fed decisions,",
    "  payrolls), but continuously quoted *before* the release rather than",
    "  read after it -- `MacroRegimeFilter`'s forward-looking counterpart.",
    "  Free, no API key, confirmed against Kalshi's live public API.",
    "",
    "All five enforce the same look-ahead firewall as `PricePanel`:",
    ...
    "",
    "**Read this before trusting any number below.** The next cell fetches",
    "fundamentals (FMP), macro (FRED), filings (SEC), options, and Kalshi",
    "independently -- each stands or falls on its own, and if any one raises",
    "it alone falls back to a synthetic stand-in rather than taking the whole",
    "cell down. FMP and FRED need registered API keys the way price data does",
    "not, so a missing FMP key alone sends the fundamentals panel to synthetic",
    "even when FRED is configured. Kalshi needs no key at all for this read",
    "path. The cell prints which source each panel actually used -- read that",
    "line before reading the tables.",
    "",
    "**Kalshi is live now, not a stand-in.** Until 2026-09-22 every live",
    "Kalshi fetch died on HTTP 429: one candlestick request per market, a few",
    "hundred back to back, against Kalshi's unauthenticated rate limit -- so",
    "this panel silently fell back to synthetic every time, here and in",
    "`run_cycle.py`. `KalshiRepository` now spaces requests",
    "(`min_request_interval`), retries 429/5xx/connection/truncated-body",
    "failures with backoff (honouring `Retry-After`), and bounds the whole",
    "fetch with `fetch_timeout` (300 s by default -- past it,",
    "`KalshiFetchTimeout` sends this panel to its stand-in rather than",
    "hanging the cell). Settled markets are cached once each under",
    "`.cache/kalshi`, so only a first run pays the full cost (roughly a",
    "minute); later runs re-fetch only markets still trading. Kalshi's",
    "public API only lists a few months of each series, so a `LIVE` Kalshi",
    "panel covers the recent past, not the full price history -- the",
    "Kalshi subsection below prints exactly how much.",
    "",
...
code(
    ...
    "",
    "",
    "KALSHI_EVENT_WINDOW_BARS = 500   # ~2 trading years -- plenty of monthly cycles",
    "",
    "",
    "def _synthetic_kalshi(panel, seed=15, lookback=KALSHI_EVENT_WINDOW_BARS):",
    "    \"\"\"A recurring monthly CPI-style event ladder over the most recent",
    "    `lookback` trading days. Each event's implied distribution is a normal",
    "    CDF sampled at seven fixed strikes (same P(X > strike) shape a real",
    "    KXCPI ladder has) whose spread narrows toward release for most",
    "    events -- the market converging as more information arrives -- but",
    "    stays wide right through close for roughly a third of them, which is",
    "    exactly the dispersion KalshiEventRegimeFilter exists to catch. This",
    "    reproduces the real successive-difference bucket math",
    "    KalshiPanel._confidence uses on an actual ladder shape, not a",
    "    single-market shortcut -- see test_kalshi.py for that math checked",
    "    against real KXCPI numbers.\"\"\"",
    "    rng = np.random.default_rng(seed)",
    "    recent_dates = panel.dates[-lookback:]",
    "    strikes = np.array([-0.4, -0.2, 0.0, 0.2, 0.4, 0.6, 0.8])",
    "    starts = pd.date_range(recent_dates[0], recent_dates[-1], freq='MS')",
    "    rows = []",
    "    for i, start in enumerate(starts):",
    "        close = start + pd.Timedelta(days=30)",
    "        days = pd.date_range(start, min(close, recent_dates[-1]",
    "                             + pd.Timedelta(days=45)), freq='D')",
    "        if len(days) == 0:",
    "            continue",
    "        center = rng.uniform(0.0, 0.4)   # where the market thinks the print lands",
    "        contested = rng.random() < 0.35",
    "        spread_open = rng.uniform(0.35, 0.50)",
    "        spread_close = rng.uniform(0.30, 0.45) if contested else rng.uniform(0.05, 0.12)",
    "        t = np.linspace(0, 1, len(days)) ** 1.3",
    "        spread = spread_open + (spread_close - spread_open) * t",
    "        spread = np.clip(spread + np.cumsum(rng.normal(0, 0.01, len(days))) * 0.03,",
    "                         0.04, 0.6)",
    "        for d, sp in zip(days, spread):",
    "            p_gt = norm.sf((strikes - center) / sp)   # P(X > strike), per contract",
    "            for k, pk in zip(strikes, p_gt):",
    "                rows.append(('cpi', f'SYN-CPI-{i}', f'SYN-CPI-{i}-T{k:+.1f}',",
    "                            float(k), d, close, float(pk), 1.0, 1.0))",
    "    frame = pd.DataFrame(rows, columns=['series', 'event_ticker', 'market_ticker',",
    "                                        'strike', 'snapshot_date', 'close_time',",
    "                                        'yes_price', 'volume', 'open_interest'])",
    ...
    "",
    "    Per panel, deliberately. These five have independent providers and",
    ...
    "# No API key at all for this one -- Kalshi's market-data endpoints are",
    "# public and unauthenticated (see qbt/kalshi.py's module docstring for",
    "# what was checked against the live service). Throttled and retried",
    "# internally; a fetch that runs past fetch_timeout raises",
    "# KalshiFetchTimeout, which lands in the stand-in branch below like any",
    "# other failure.",
    "kalshi, _src_k = _panel_or_stand_in(",
    "    'kalshi',",
    "    lambda: KalshiRepository(cache_dir='.cache/kalshi').fetch(START, END),",
    "    lambda: _synthetic_kalshi(panel))",
    "",
...
code(
    ...
    "print()",
    "print('kalshi implied confidence + days to close, nearest open event per series:')",
    "print(pd.DataFrame({'confidence': kalshi.snapshot(mid),",
    "                    'days_to_close': kalshi.days_to_close(mid)}))",
    "print()",
    ...
    "leak_free_k = (kalshi.as_of(mid).frame['snapshot_date'] <= mid).all()",
    "print(f'kalshi.as_of({mid.date()}) contains no quote taken later: {leak_free_k}')",
)
...
md(
    "### Kalshi: forward-looking event odds, graphed",
    "",
    "Everything above reacts to data *after* it becomes public. `KalshiPanel`",
    "is the one exception: Kalshi continuously prices market odds on a",
    "scheduled release -- CPI, a Fed decision, payrolls -- for weeks *before*",
    "it happens, so a strategy can react to the run-up, not just the",
    "aftermath. What it actually derives per event is the ladder's *implied",
    "confidence*: the largest single bucket of probability mass after taking",
    "successive differences across strikes (`KalshiPanel._confidence`) -- a",
    "market split evenly across many strikes reads as low confidence, one",
    "where a single bucket dominates reads as high confidence. See",
    "`qbt/kalshi.py`'s module docstring for the live-liquidity survey behind",
    "which series are actually worth reading this way.",
)

md(
    "#### The raw ladder first, before it collapses to one number",
    "",
    "`_confidence` is a derived statistic -- worth seeing what it's derived",
    "*from* before trusting the history chart below. A CPI event isn't one",
    "price; it's a set of separate yes/no markets, one per strike (about 14",
    "on a real `KXCPI` ladder, 7 on the stand-in), each quoting",
    "`P(CPI change > strike)`. The chart below plots that ladder directly --",
    "yes price against strike -- at four points in one CPI event's life",
    "(real if the source line in the fetch cell said `LIVE`, the stand-in",
    "otherwise), from its first snapshot to the close, with the same confidence",
    "number `KalshiPanel._confidence` derives from each snapshot given in the",
    "legend. A market still undecided spreads its curve into a shallow,",
    "gradual slope -- probability mass smeared across several strikes. As",
    "the close approaches and the market converges on an answer, the curve",
    "should visibly steepen into one sharp step -- most of the mass now",
    "sitting in a single gap between two strikes, which is exactly the",
    "shape `_confidence` is measuring.",
)

code(
    "event_frame = kalshi.frame[kalshi.frame['series'] == 'cpi']",
    "# the event with the most recorded snapshots -- a full lifecycle rather",
    "# than one truncated by the edge of KALSHI_EVENT_WINDOW_BARS",
    "target_ticker = (event_frame.groupby('event_ticker')['snapshot_date']",
    "                 .nunique().idxmax())",
    "ladder_hist = (event_frame[event_frame['event_ticker'] == target_ticker]",
    "               .sort_values('snapshot_date'))",
    "close_time = ladder_hist['close_time'].iloc[0]",
    "snap_dates = ladder_hist['snapshot_date'].unique()",
    "picks = snap_dates[np.linspace(0, len(snap_dates) - 1, 4).round().astype(int)]",
    "",
    "fig, ax = plt.subplots(figsize=(7.5, 4.5))",
    "for d, shade in zip(picks, np.linspace(0.35, 0.9, len(picks))):",
    "    day_rows = ladder_hist[ladder_hist['snapshot_date'] == d].sort_values('strike')",
    "    days_left = (pd.Timestamp(close_time).normalize() - pd.Timestamp(d)).days",
    "    confidence = KalshiPanel._confidence(day_rows)",
    "    ax.plot(day_rows['strike'], day_rows['yes_price'], marker='o', ms=5, lw=1.8,",
    "            color=plt.cm.Purples(shade),",
    "            label=f'{days_left}d to close -- confidence {confidence:.0%}')",
    "ax.set_ylim(-0.02, 1.02)",
    "ax.set_xlabel('strike (CPI MoM change threshold)')",
    "ax.set_ylabel('yes price = P(CPI change > strike)')",
    "ax.set_title(f'{target_ticker}: one ladder, four snapshots from open to close')",
    "ax.legend(fontsize=8.5, title='snapshot')",
    "plt.tight_layout(); plt.show()",
)

md(
    "Now zoom out from one ladder to the full history: the next cell walks",
    "the `cpi` series day by day -- over the window the Kalshi panel",
    "actually covers, which for live data is only the few months Kalshi's",
    "API lists -- and plots the confidence path",
    "directly -- shading in exactly where `KalshiEventRegimeFilter`",
    "(`min_confidence={'cpi': 0.40}, horizon_days=3`, the same defaults",
    "`run_cycle.py`'s live `STRATEGY` uses) would actually fire, computed",
    "from the filter object itself rather than a hand-copied threshold, so",
    "the shading can't silently drift from what the filter really does.",
)

code(
    "# Only the stretch the Kalshi panel actually covers: live data reaches",
    "# back just a few months, and a 500-day window over that would be",
    "# mostly empty chart.",
    "_k_start = kalshi.frame['snapshot_date'].min()",
    "print(f'Kalshi panel ({_src_k}) covers {_k_start.date()} to '",
    "      f\"{kalshi.frame['snapshot_date'].max().date()}\")",
    "hist_dates = panel.dates[panel.dates >= _k_start][-KALSHI_EVENT_WINDOW_BARS:]",
    "hist = pd.DataFrame({",
    "    'confidence': [kalshi.snapshot(d).get('cpi', np.nan) for d in hist_dates],",
    "    'days_to_close': [kalshi.days_to_close(d).get('cpi', np.nan) for d in hist_dates],",
    "}, index=hist_dates)",
    "covered = hist['confidence'].notna().sum()",
    "print(f'{covered} of {len(hist)} trading days have an open cpi event to read '",
    "      f'({covered / len(hist):.0%}) -- the rest sit between one release closing '",
    "      \"and the next month's ladder opening.\")",
    "hist.dropna().tail()",
)

code(
    "kalshi_gate = KalshiEventRegimeFilter(momentum, min_confidence={'cpi': 0.40},",
    "                                      horizon_days=3)",
    "floor = kalshi_gate.min_confidence['cpi']",
    "would_block = ((hist['days_to_close'] <= kalshi_gate.horizon_days)",
    "              & (hist['confidence'] < floor)).fillna(False)",
    "",
    "fig, ax = plt.subplots(figsize=(11, 4))",
    "ax.plot(hist.index, hist['confidence'], color='#26215C', lw=1.2,",
    "        label='implied confidence (largest ladder bucket)')",
    "ax.axhline(floor, color='#D85A30', ls='--', lw=1, label=f'cpi floor ({floor:.0%})')",
    "ax.fill_between(hist.index, 0, 1, where=would_block.to_numpy(), color='#D85A30',",
    "                alpha=0.15, transform=ax.get_xaxis_transform(),",
    "                label='KalshiEventRegimeFilter would block here')",
    "ax.set_ylim(0, 1)",
    "ax.set_ylabel('implied confidence')",
    "ax.set_title('Kalshi-implied CPI confidence over time, and where the regime filter fires')",
    "ax.legend(loc='lower left', fontsize=8.5)",
    "plt.tight_layout(); plt.show()",
    "",
    "n_block = int(would_block.sum())",
    "print(f'{n_block} of {len(hist)} trading days would trigger the block '",
    "      f'({n_block / len(hist):.1%}) -- always in the last {kalshi_gate.horizon_days} '",
    "       'trading days before a release the market had not converged on.')",
)

md(
    "Same role as `MacroRegimeFilter` in the previous subsection -- de-risk",
    "the whole book, change nothing about which names get picked -- just",
    "reacting to a different kind of regime read. Wired into a backtest the",
    "same way. With a `LIVE` Kalshi panel, the gate only has readings for the",
    "few months the panel covers -- before that it passes through untouched,",
    "so any gap between the two rows below comes from that recent stretch",
    "alone. Treat it as a wiring check, not evidence of an edge.",
)

code(
    "kalshi_strategies = {",
    "    'momentum':             momentum,   # baseline, same object as above",
    "    'momentum_kalshi_gated': kalshi_gate,",
    "}",
    "kalshi_results = {",
    "    name: Backtester(",
    "        panel=panel, strategy=s, kalshi=kalshi,",
    "        risk_gate=RiskGate(**GATE), cost_model=COSTS, execution=EXEC,",
    "        rebalance=REBAL, initial_equity=INITIAL_EQUITY,",
    "    ).run()",
    "    for name, s in kalshi_strategies.items()",
    "}",
    "compare(kalshi_results)[cols].round(3)",
)
...
md(
    "### A position it can't price stops the plan",
    "",
    "`PortfolioState.equity()` counts a held symbol with no price as $0.",
    "Planning on that understates equity by the whole position -- every",
    "weight, the vol-target scale, and the drawdown the risk gate measures",
    "are then wrong, and a tripped drawdown breaker means *sell*. So",
    "`LiveSignalRunner.plan()` checks `state.unpriced(prices)` first: if any",
    "held position has no usable price (missing, zero or infinite) on the",
    "decision bar, it returns an empty plan with a warning naming the",
    "symbols, and `run_cycle.py` aborts the cycle with nothing sent. This is",
    "the second guard behind the fetch-time one in section 1: the fetch",
    "drops a whole unfinished bar; this catches anything that still gets",
    "through, such as one held name blank on an otherwise complete day.",
    "Symbols you don't hold can be blank without blocking anything.",
)

code(
    "# Blank the decision-bar close for one name `state` holds, and plan again.",
    "_held = state.shares[state.shares.abs() > 0].index[0]",
    "_blank = panel.close.copy()",
    "_blank.loc[_blank.index[-1], _held] = np.nan",
    "refused = runner.plan(PricePanel(close=_blank), state)",
    "print(f'blanked {_held} on {_blank.index[-1].date()}: '",
    "      f'{len(refused.intents)} intents, risk gate ran: {refused.decision is not None}')",
    "for w in refused.warnings:",
    "    print('  WARNING:', w)",
    "print(f'state.equity() would have said {state.equity(_blank.iloc[-1]):,.0f} '",
    "      f'against {state.equity(panel.last_close()):,.0f} with the price present')",
)
...
md(
    ...
    "",
    "**Two data guards come first.** Before any of the stops below, the",
    "price fetch drops an unfinished last bar (section 1), and the planner",
    "refuses to plan when a held position has no price (section 10). Both",
    "exist because a blank price once valued a live account at its cash",
    "alone and tripped the drawdown breaker on a loss that never happened.",
    "Both show up in the audit log (`price_bar_trimmed`, `plan_warnings`).",
    "",
